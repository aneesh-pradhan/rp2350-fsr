...
class SerialProcessor {
 public:
  ...
  void CheckAndMaybeProcessData() {
    ...
    // USB CDC (RP2040/RP2350): avoid blocking reads or newline-dependent parsing.
    // Process a single command byte and never wait for more data.
    switch (cmd) {
      case 'o':
      case 'O':
        UpdateOffsets();
        // Flush any leftover bytes (e.g. line endings) so they don't become commands.
      ...
      case '0' ... '9': // Case ranges are non-standard but work in gcc.
        ReadThresholdCommand(cmd);
        break;
      default:
        // Unknown command: flush leftovers and ignore.
        while (Serial.available()) { Serial.read(); }
        break;
    }
  }

  void ReadThresholdCommand(char first_char) {
  buffer_[0] = first_char;
  size_t idx = 1;

  unsigned long start = millis();
  while ((millis() - start) < 10 && idx < kBufferSize - 1) {
    if (Serial.available()) {
      char c = Serial.read();
      if (c == '\n' || c == '\r') {
        break;
  ...
  if (sensor_threshold < 0 || sensor_threshold > 1023) {
    return;
  }
